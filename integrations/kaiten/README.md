...
- **Клиентский троттлинг** (opt-in): env `KAITEN_RATE=min|max` — пауза перед каждым запросом (`min`=0.2с, `max`=0.6с ≈100/мин). Для bulk-вызывателей (перебор сотен карточек). Дефолт — без паузы (интерактивные вызовы пейсятся сами).
