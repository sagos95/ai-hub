...
Скрипты лежат внутри плагина. Резолви путь через `CLAUDE_PLUGIN_ROOT`, чтобы команда
работала из любого репозитория (не пиши относительные `integrations/buildin/scripts/...`):
...
```bash
bash "$BUILDIN_DIR/buildin-shadow.sh" update "<page_id>" "<title>" "<краткое саммари, 1-2 предложения>" "<parent_id>"
```
...
- Пустой результат → Контент может быть в дочерних страницах:
  ```bash
  bash "$BUILDIN_DIR/buildin-nav.sh" children "<page_id>"
  ```
