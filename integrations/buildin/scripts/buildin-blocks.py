...
Usage:
    buildin-blocks.py <page_id> <space_id> <now_ms> <user_id> <blocks_json> [after_block_id]

Если передан after_block_id — первый блок верхнего уровня вставляется после него
(режим insert-blocks-after), иначе блоки добавляются в конец страницы.
"""
import json
import sys
import uuid
...
after_block_id = sys.argv[6] if len(sys.argv) > 6 else None
...
def emit(block, parent_id, after=None):
