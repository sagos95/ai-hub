...
Обычный PR из ветки внутри репозитория может создать только тот, у кого есть write-доступ (коллаборатор). Если тебя нет в коллабораторах — это норма для публичного репозитория — форкни `sagos95/ai-hub` и пришли PR из своего форка. Это стандартная опенсорс-модель fork-and-pull: чужой код изолирован и не выполняется в нашем CI, пока мейнтейнер его не одобрит.
